...
| **FV-003** | Write commandable output at priority | analog-output present-value, write via MQTT `…/set` at priority 12 | device value changes; lower-priority writes rejected | round-trip (`rw-soak-e2e.sh`); a failed write surfaces a `Write failed because:` warn — verified against an aborting device (`write-depth-e2e.sh`); **priority-rejection / NULL relinquish: manual** (rw-fleet uses plain present-value; a commandable profile would automate them) | ☐ |
...
`mqtt-env.sh` (source it for `$MQTT_ARGS`/`$CW_CURL_CA` — dev plain or production TLS; see R7).
...
## R7. Running the battery against a production Connectware

The battery defaults to the **dev** install — plain `mqtt://localhost:1883`, `admin/admin`,
self-signed API (`curl -k`). A production Connectware differs only in **transport and auth**,
not in adapter behaviour: the data plane is **TLS** (`mqtts://…:8883`), the API is
**CA-signed**, and broker/API access needs a real Connectware user (or mTLS client cert).

`qa/tools/mqtt-env.sh` bridges it: source it and it builds `$MQTT_ARGS` (mosquitto flags,
plain or TLS) and `$CW_CURL_CA` (the curl CA flag) from env. `write-depth-e2e.sh` is wired
this way as the reference; the other e2e scripts adopt it with the same two changes —
`. "$DIR/../tools/mqtt-env.sh"`, then swap `mosquitto_* -h … -p … -u … -P …` / `curl -k`
for `$MQTT_ARGS` / `$CW_CURL_CA`.

| Var | Example | Meaning |
|---|---|---|
| `CW_HOST` | `cw.example.com` | gateway host (API + broker) |
| `CW_USER` / `CW_PASS` | a CW service user | API + broker credentials (needs service + broker permissions) |
| `CW_CA` | `/certs/cw-ca.pem` | CA to verify the HTTPS API (drops `-k`) |
| `MQTT_SCHEME` | `mqtts` | switch the data plane to TLS (port defaults to 8883) |
| `MQTT_CA` | `/certs/cw-ca.pem` | CA to verify the broker |
| `MQTT_CLIENTCERT` / `MQTT_CLIENTKEY` | `/certs/client.{pem,key}` | **optional** mTLS — cert CN must be in the CW CA chain |
| `SIM_HOST` | real device IPs | BACnet devices must be reachable **from the protocol-mapper container** |

```bash
export CW_HOST=cw.example.com CW_USER=qa CW_PASS=… CW_CA=/certs/cw-ca.pem \
       MQTT_SCHEME=mqtts MQTT_CA=/certs/cw-ca.pem MQTT_HOST=cw.example.com
qa/e2e/write-depth-e2e.sh        # same script, now over the production TLS broker + CA-verified API
```

**Why test prod when the adapter logic is identical to dev:** to prove the TLS/auth/cert
path itself, and that **large CC-4157 messages survive the production data plane** — TLS
framing, QoS, retained-message and broker max-packet limits that a 20k-object payload can
stress in ways plain localhost never does. Run **P1** (`spectrum`) and **P2** (`func-array`,
`discovery`) against prod specifically for that.
